...
class DeepIntegration
{
    ...
    public function init()
    {
        ...
        add_filter('fluentcrm_advanced_report_providers', function ($providers) {
            $providers['lifterlms'] = [
    ...
    public function syncStudent($userId, $contactStatus = 'subscribed', $tags = [], $lists = [], $sendDoubleOptin = true, $forceSync = false)
    {
        ...
        $taxonomies = Helper::getTagsCatsByCourseIds($courseIds);
    ...
    public function maybeReSyncStudent($userId, $courseId, $hook)
    {
        ...
        if ($hook == 'llms_user_added_to_membership_level') {
            ...
            $itemIds = ContactRelationItemsModel::where('relation_id', $contactRelation->id)
                ->where('item_type', 'course')
                ->get()->pluck('item_id')
                ->toArray();

            $taxonomies = Helper::getTagsCatsByCourseIds($itemIds);
            $extraData = [
                'commerce_taxonomies' => json_encode(array_map('strval', $taxonomies))
            ];

            $contactRelation->recalculate(['first_order_date', 'last_order_date', 'total_order_count'], 'course', $extraData);
            return true;
        } else {
