...
class WooSmartCodeParse
{
    ...
    {
        add_filter('fluentcrm_smartcode_fallback_callback_woo_customer', array($this, 'parseCustomer'), 10, 4);
        add_filter('fluentcrm_smartcode_fallback_callback_woo_order', array($this, 'parseCurrentOrder'), 10, 4);
        add_filter('fluentcrm_smartcode_fallback_callback_woo_last_order', array($this, 'parseLastOrder'), 10, 4);

        add_filter('fluentcrm_extended_general_smart_codes', array($this, 'pushGeneralCodes'));
    ...
    {
        $order_items = $order->get_items(apply_filters('woocommerce_purchase_order_item_types', 'line_item'));
        $hash = apply_filters('fluentcrm_woo_product_review_hash', '#tab-reviews');
                            ...
                            <div style="padding: 0px 10px;" class="fc_btn">
                                <a href="<?php echo esc_url_raw($product->get_permalink() . $hash); ?>"
                                   class="fc_review_btn">
                                    ...
                                    <![endif]-->
                                    <span
                                        style="mso-text-raise:15pt;"><?php echo apply_filters('fluentcrm_review_button_text', esc_html__('Leave a review', 'fluentcampaign-pro')); ?></span>
                                    <!--[if mso]>
